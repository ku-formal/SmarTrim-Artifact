...
let set_nr_penalty no_regressions alarms_patch patch =
  let b1 = BatSet.for_all Kind.(fun (kind,_,_,_) -> kind <> RE_EL && kind <> RE) alarms_patch in
...
= fun solutions file ->
  let _ = assert (Solc.is_compilable (get_solc_ver ()) file) in
  let _ = assert (List.length solutions <= 1) in
  let cand_dir = Filename.dirname file in (* root/candidates/cand_1/patch.sol => root/candidates/cand_1 *)
  let best_cand_dir =
  ...
  let j = Yojson.Basic.from_file (get_jr cand_dir) in
...
let train tr_set =
  if !R.no_learn then (Py.none, None)
  else
  ...
  List.exists (fun p ->
    List.exists (fun c ->
      ...
      | Atom (InsertLine (l1, s1, is_bc1)), Atom (InsertLine (l2, s2, is_bc2)) ->
        ...
        let fkey1, fkey2 = Func.fkey f1, Func.fkey f2 in
...
let compute_penalty global comps _solutions patch =
...
let score model_off model_on tr_set comps patch =
  if !R.no_learn then -. (Patch.length2 patch)
  else
...
module Workset = struct
  ...
  module OrderedType = struct
    type t = elem
    let compare (_,_,cost1) (_,_,cost2) = Stdlib.compare cost1 cost2
  end
...
= fun global lines model_off model_on tr_set comps solutions patch (workset,explored) ->
  ...
  if BatSet.mem src_str explored then
    ...
    let candroot = get_cands_dir !outdir ^ "/" ^ "cand_" ^ string_of_int id in
    ...
    if exception_encountered then
      ...
      (workset, explored)
    else if not (Solc.is_compilable (get_solc_ver ()) patch_fullfname) then
...
let print_log _global _pgm file patch cur_cost model_off model_on comps =
...
let print_log_err _global _pgm _file _patch err_msg =
  if err_msg = "Solution Inv Does Not Hold" then
    print_endline ("- " ^ err_msg ^ " !\n")
  else
    print_endline ("- Verification error !\n")

let print_log2 global pgm _file patch (feat, reward) fit_score _comps =
...
let rec loop : Global.t -> (string list * Pgm.t) -> alarms -> int -> float -> patch_comp list ->
...
= fun global (lines,pgm) alarms_org min_alarm_num min_patch_size comps model_off model_on tr_set (workset, explored) (solutions, knowledges) ->
  ...
  let timeout = Unix.gettimeofday () -. !Profiler.start_real > float_of_int !R.loop_timeout in
  ...
  | Some (file, patch, cur_cost) ->
    let _ = if !debug = "repair" then print_log global pgm file patch cur_cost model_off model_on comps in
    ...
    if not (no_error) then (* verifier runtime exception check *)
      let _ = if !debug = "repair" then print_log_err global pgm file patch err_msg in
      ...
      let alarms_patch = file |> Filename.dirname |> get_vr |> FixTarget.get_alarms in
      let no_regressions = has_no_regressions (get_rr !outdir) (file |> Filename.dirname |> get_rr) in
      ...
      let (model_off, tr_set, model_on, fit_score) =
        if not off_unuseful then (model_off, tr_set, model_on, fit_score)
          ...
          let _ = if !debug = "repair" then debug_invalidate (snd new_data) in
          let model_off = Py.none in
          let tr_set = BatSet.filter (fun (feat,_score) -> BatList.sum feat = 1) tr_set in
      ...
      let _ = if !debug = "repair" then print_log2 global pgm file patch new_data fit_score comps in
      loop global (lines,pgm) alarms_org min_alarm_num min_patch_size comps model_off model_on tr_set (workset,explored) (solutions,knowledges)


let get_reportfile () =
  if !R.report="" then (* TODO : do something proper when error happens, e.g., no report exists *)
    (ignore (Profiler.run "[STEP] Run analyzer" (RunTool.verify_s0 !input !outdir));
     get_vr !outdir)
  else
     let _ = assert (Sys.command ("cp " ^ !R.report ^ " " ^ get_vr !outdir) = 0) in
     get_vr !outdir


let run : Global.t -> string list -> Pgm.t -> solution list
