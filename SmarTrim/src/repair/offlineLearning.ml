...
type feature = {
  def_typs : Typ.t BatSet.t;
  use_typs : Typ.t BatSet.t;
  payable : bool;
  send_eth : bool;
  destruct : bool;
  contain_loop : bool;
...
let to_string_typs typs =
  string_of_set ~first:"{" ~last:"}" ~sep:"," Typ.to_string typs
...
let add_def_typs : Typ.t BatSet.t -> feature -> feature
= fun typs feat -> {feat with def_typs = BatSet.union typs feat.def_typs}

let add_use_typs : Typ.t BatSet.t -> feature -> feature
...
let rec def_of : lv -> Var.t BatSet.t
= fun lv ->
  match lv with
  | Var (v,vinfo) -> BatSet.singleton (v, vinfo.vtyp)
  | MemberAccess (_,x,xinfo,_) -> BatSet.singleton (x, xinfo.vtyp)
...
let feature_map : Global.t -> Func.t -> feature
= fun global func ->
  let cfg = Func.cfg func in
  let nodes = MakeCfg.nodesof cfg in
  let feat = if Func.is_payable func then add_payable empty_feature else empty_feature in
  List.fold_left (fun acc node ->
    let stmt = Cfg.find_stmt node cfg in
    match stmt with
    | Assign (lv,_,_) ->
      let vars = BatSet.filter (fun x -> List.mem x global.global_vars) (def_of lv) in
      let typs = BatSet.map get_type_var vars in
      add_def_typs typs acc
    | Assume (e,_) ->
      let vars = BatSet.filter (fun x -> List.mem x global.global_vars) (var_exp e) in
      let typs = BatSet.map get_type_var vars in
    ...
    | Call (_,Lv (MemberAccess (e,fname,_,_)),_,_,_,_)
      ...
      add_send_eth acc
    | Call (_, Lv (MemberAccess (e,"call",_,_)), _, Some _, _, _)
      when Typ.is_address_kind (get_type_exp e) ->
      add_send_eth acc
    | Call (_,Lv (Var (fname,_)),args,_,_,_)
      when List.mem fname ["selfdestruct";"suicide"] ->
      let _ = assert (List.length args = 1) in
      add_destruct acc
    | _ when Cfg.is_loophead node cfg -> add_contain_loop acc
    | _ -> acc
  ) feat nodes

let to_string_bops bops =
  string_of_set ~first:"{" ~last:"}" ~sep:"," Bop.show bops

module Abs_PatchComp = struct
  type t' =
    | Abs_Insert of feature * (* function *)
                    Typ.t BatSet.t (* patch *)
    | Abs_Replace of feature * (* function *)
                     Bop.t BatSet.t * Typ.t BatSet.t * (* original *)
                     Bop.t BatSet.t * Typ.t BatSet.t   (* patch *)
...
let typs_stmt : Global.t -> Stmt.t -> Typ.t BatSet.t
= fun global stmt ->
  match stmt with
  | Assign (lv,_,_) ->
    BatSet.filter (fun v -> List.mem v global.global_vars) (var_lv lv)
    |> BatSet.map snd
  | Assume (e,_) ->
    BatSet.filter (fun v -> List.mem v global.global_vars) (var_exp e)
    |> BatSet.map snd
  | _ -> assert false

let rec bops_exp : exp -> Bop.t BatSet.t
= fun exp ->
  match exp with
  | V Int _ | V Frac _ | Str _ -> BatSet.empty
  | Lv lv -> bops_lv lv
  | Cast (_,e) -> bops_exp e
...
and bops_lv : lv -> Bop.t BatSet.t
= fun lv ->
  match lv with
  | Var _ -> BatSet.empty
  | MemberAccess (e,_,_,_) -> bops_exp e
...
let bops_stmt : Global.t -> Stmt.t -> Bop.t BatSet.t
= fun _global stmt ->
  match stmt with
  | Assign (lv,e,_) -> BatSet.union (bops_lv lv) (bops_exp e)
  | Assume (e,_) -> bops_exp e
  | _ -> assert false
...
= fun global atomic ->
  ...
  | Replace (l,e1,e2) ->
    ...
    let typs1 = BatSet.filter (fun v -> List.mem v global.global_vars) (var_exp e1) |> BatSet.map snd in
    let bops2 = bops_exp e2 in
    let typs2 = BatSet.filter (fun v -> List.mem v global.global_vars) (var_exp e2) |> BatSet.map snd in
    Abs_PatchComp.Abs_Replace (feat,bops1,typs1,bops2,typs2)

  | AddModifier (fkey,"@nonReentrant",_) ->
    let feat = feature_map global (FuncMap.find fkey global.fmap) in
    Abs_PatchComp.Abs_NR feat

  | AddModifier (fkey,mname,_) ->
    let f_feat = feature_map global (FuncMap.find fkey global.fmap) in
    let m_feat = feature_map global (FuncMap.find_modifier_by_name global.fmap mname) in
...
let abstract : Global.t -> Pgm.t -> patch -> abs_patch
= fun global _pgm patch -> List.map (abstract' global) patch
...
let knowledges_to_json : Global.t -> Pgm.t -> knowledge BatSet.t -> Yojson.Basic.t
= fun _global _pgm knowledges ->
  let knowledges = BatSet.to_list knowledges in
  `Assoc [("src", `List [`String (Filename.basename !input)]);
          ("knowledge", `List (List.map knowledge_to_json knowledges))]

let store_new_knowledges : Global.t -> Pgm.t -> knowledge BatSet.t -> unit
= fun global pgm knowledges ->
  let json = knowledges_to_json global pgm knowledges in
  let f = open_out (!outdir ^ "/" ^ "new_knowledge.json") in
...
let combine_knowledges () =
  let kdir = get_knowledge !outdir in
  if not (Sys.file_exists kdir) then ()
  else
    let oldf = Yojson.Basic.from_file (get_knowledge !outdir) in
    let newf = Yojson.Basic.from_file (!outdir ^ "/" ^ "new_knowledge.json") in

    let old_src = oldf |> J.member "src" |> J.to_list in
    let new_src = newf |> J.member "src" |> J.to_list in

    let old_k = oldf |> J.member "knowledge" |> J.to_list in
    let new_k = newf |> J.member "knowledge" |> J.to_list in

    let json = `Assoc [("src", `List (old_src @ new_src));
                       ("knowledge", `List (old_k @ new_k))] in

    let f = open_out (!outdir ^ "/" ^ "combined_knowledge.json") in
    Printf.fprintf f "%s" (Yojson.Basic.pretty_to_string json);
    close_out f
...
let to_feature : Global.t -> Pgm.t -> patch_comp list -> Yojson.Basic.t -> OnlineLearning.feature
= fun global _pgm comps json ->
  let strip j = match j with `String s -> s | _ -> assert false in
  let abs_patch = json |> J.member "abs_patch" |> J.to_list |> List.map strip in
...
= fun json -> json |> J.member "score" |> J.to_float

let load_knowledge : Global.t -> Pgm.t -> patch_comp list -> Yojson.Basic.t -> tr_data
= fun global pgm comps knowledge ->
  ...
  (* let _ =
    if not (List.for_all (fun n -> (n = 0)) feature) then
      let abs_patch = knowledge |> J.member "abs_patch" |> J.to_list |> to_string_abs_patch2 in
...
let copy_knowledge_src () =
  ...
  if ksrc = "" then ()
    ...
    assert (Sys.command ("cp " ^ ksrc ^ " " ^ !outdir ^ "/" ^ "knowledge.json") = 0)

let load_knowledges : Global.t -> Pgm.t -> patch_comp list -> tr_set
= fun global pgm comps ->
  ...
  if not (Sys.file_exists kdir) then BatSet.empty
    ...
    let knowledges = match J.member "knowledge" json with `List lst -> lst | _ -> assert false in
