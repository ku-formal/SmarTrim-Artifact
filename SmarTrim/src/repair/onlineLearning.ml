...
let reward_alarm cand_file patch lpenalty_weight =
  let org_a = (get_vr !outdir) |> FixTarget.get_alarms |> BatSet.cardinal in
  let cur_a = BatSet.cardinal (get_vul_alarms cand_file) in
  let org_reg_report = get_rr !outdir in
  let cur_reg_report = cand_file |> Filename.dirname |> get_rr in
