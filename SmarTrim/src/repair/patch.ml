...
module PatchComp = struct
  type t' =
    | InsertLine of int * Stmt.t * bool    (** if bool is true, bound check *)
    | Move of (int * Stmt.t) list * int * Stmt.t    (** int = line; Move (lst,l,s) : move statements at 'lst' forward to the statement at l *)
    | Replace of int * exp * exp    (** Replace (l,e,e')    : e with e' at line l *)
    | ChangeToCnstr of Fkey.t                     (** ChangeToCnstr(fkey) : change a nomral function to cnstr *)
    | AddModifier of Fkey.t * string * int        (** AddModifier(f,name) : add a modifier to a function *)
    | ElseRevert of exp * Stmt.t * Stmt.t * Ifinfo.t
  ...
  = fun a ->
    ...
    | ChangeToCnstr fkey -> "Change " ^ Fkey.to_string fkey ^ " to constructor"
    | AddModifier (fkey,s,l) -> "Add " ^ s ^ " to " ^ Fkey.to_string fkey ^ " (line " ^ string_of_int l ^ ")"
  ...
  = fun a ->
    ...
    | ChangeToCnstr _fkey -> BatSet.singleton (-1)
  ...
  let compare comp1 comp2 =
    ...
    | AtomLst lst1, AtomLst lst2 -> List.compare compare' lst1 lst2
...
let lines_of : patch -> int BatSet.t
= fun patch ->
...
let regex_money n (eth_unit, unit_value) =
  let n' = (float_of_string (Z.to_string n)) /. unit_value in
  if n' < 1. then
...
(* e.g., 100 => 1e2 *)
let enumber n =
  if Z.equal (Z.rem n (Z.of_int 10)) Z.zero (* n mod 10 = 0 *)
     && BatString.starts_with (Z.to_string n) "1" then
    let zero_num = snd (BatString.split ~by:"1" (Z.to_string n)) |> BatString.length in
    ["1e" ^ string_of_int zero_num]
  else []

let regex_int n =
  (* try hexadecimal first, because trying decimal would result in changes somehow *)
  ["0x" ^ Z.format "%x" n; Z.to_string n;
   regex_money n ("wei", 1.); regex_money n ("szabo", 1e12);
   regex_money n ("finney", 1e15); regex_money n ("ether", 1e18)]
  @ (enumber n)
  @
  (if Z.equal (Z.rem n (Z.of_int 604800)) Z.zero then
     [Z.to_string (Z.div n (Z.of_int 604800)) ^ rspace  ^ "weeks"]
   else if Z.equal (Z.rem n (Z.of_int 86400)) Z.zero then
     [Z.to_string (Z.div n (Z.of_int 86400)) ^ rspace  ^ "days"]
   else [])
...
= fun exp ->
  match exp with
  | V (Int n) -> regex_int n
  | V (Frac n) -> [Q.to_string n]
  | V (Bool true) -> ["true"]
  | V (Bool false) -> ["false"]
  | V _ -> raise NotImplemented
  | Str s -> [s]
  | Lv lv -> regex_lv lv
  | Cast (typ,e) ->
  ...
  | CondTemp _ -> raise NotImplemented
  | AssignTemp _ -> raise NotImplemented
...
and regex_typ : Typ.t -> string list
= fun typ ->
  match typ with
  ...
  | Array (_,None) -> raise NotImplemented
  | Array (_,Some _) -> raise NotImplemented (* to_string_typ typ ^ "[" ^ string_of_int n ^ "]" *)
  | Struct lst -> [string_of_list ~first:"" ~last:"" ~sep:"." Vocab.id lst]
  | TupleType _ | FuncType _ -> assert false
  | Void -> assert false

and regex_etyp : Typ.e -> string list
= fun etyp ->
  match etyp with
  | Address -> ["address"]
  ...
  | Bytes _ -> raise NotImplemented (* ["bytes" ^ string_of_int n] *)
...
let inc_dec op x =
  (* if-conditional checking is important for performance *)
  (* leak/0x34BC9A332350A41374aa650f1152C8D88e35a878_1.sol, leak/0xf7a3c5e0310ed2bafde8e782f43ab38c1cfa279d_1.sol *)
...
let to_string_stmt_wrapper (stmt : Stmt.t) =
  match stmt with
  | Assume (e,_loc) -> "require" ^ "(" ^ to_string_exp ~report:true e ^ ")" ^ ";"
  | _ -> Lang.to_string_stmt ~report:true stmt

let should_use_safemath : int -> string list -> Stmt.t -> bool
= fun loc lines stmt ->
  let str = BatList.at lines (loc-1) |> (fun s -> try fst (BatString.split s ~by:"//") with Not_found -> s) in
  let contain_forloop _loc _lines = BatString.exists str "for" in
  let is_plus_one_guard (stmt : Stmt.t) =
    match stmt with
    | Assume (BinOp (GEq, BinOp (Add,_,V Int n,_),_,_),_) when Z.equal n Z.one -> true
    | _ -> false in
...
let insert_safeguard : Stmt.t -> string -> string
= fun stmt x ->
  let extract_exp (stmt : Stmt.t) =
    match stmt with
    | Assume ((BinOp (GEq, (BinOp (Add,_,V Int n,_) as exp), _, _)), _) when Z.equal n Z.one -> exp
    | _ -> assert false in
  let exp_to_safeguard exp =
    match exp with
    | BinOp (Add,e1,e2,einfo) ->
      (match einfo.typ with
       | EType (UInt n) ->
...
= fun a lines ->
  ...
  | InsertLine (loc, (Assign (lv,_,_) as stmt), _bc)
    when BatString.starts_with (to_string_lv lv) "tmp__" ->
    BatList.modify_at (loc-1) (fun x ->
      space_of x ^ Typ.to_string (get_type_lv lv) ^ " " ^ to_string_stmt_wrapper stmt
      ^ " /* <FIX> Insert */" ^ "\n" ^ x
    ) lines
  ...
  | Move (lst,loc,_) ->
    ...
    let lines' = List.fold_left (fun acc (l',_) ->
                   BatList.modify_at (l'-1) (fun x -> space_of x ^ "/* <FIX> Move: " ^ strip_space x ^ " */") acc
                 ) lines lst in
    (* move lst in front of the statement at loc *)
    BatList.modify_at (loc-1) (fun x ->
      let targets = List.map (fun (_,s) -> space_of x ^ to_string_stmt_wrapper s ^ " /* <FIX> Move */") lst in
  ...
  | ChangeToCnstr fkey ->
    BatList.fold_left (fun acc x ->
      acc @ [Str.global_replace (Str.regexp ("function" ^ "\\( \\)*" ^ fkey.func)) " /* <FIX> Change to Cnstr: */ constructor" x]
    ) [] lines
