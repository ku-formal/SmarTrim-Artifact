...
type t = Kind.t * string * int * string option
and fix_target = t
...
let get_kind_target (a,_,_,_) = a
let get_fkey_target (_,b,_,_) = b
let get_line_target (_,_,c,_) = c
let get_exp_target (_,_,_,d) = d

(* TODO: refactoring *)
let str_to_typs' : string -> Typ.t
= fun str ->
...
let csv_header = Report.csv_header
...
= fun reportfile ->
  ...
  List.fold_left (fun acc row ->
    ...
    let kind = Kind.of_string (Csv.Row.find row "kind") in
...
= fun reportfile ->
  ...
  let _ = assert (BatSet.for_all (fun ((kind : Kind.t),_,_,_) -> kind != NO_EFFECT && kind != ASSIGN_CONST && kind != DEAD) all) in
  all

let get_alarms2 : string -> fix_target BatSet.t * fix_target BatSet.t
= fun reportfile ->
  let all = get_alarms reportfile in
  let all_wo_leak = BatSet.filter (fun ((kind : Kind.t),_,_,_) -> kind != EL) all in
  (all, all_wo_leak)
