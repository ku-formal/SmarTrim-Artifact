...
let rec constrained : vformula -> Var.t -> bool
= fun vf target ->
  match vf with
  | VTrue | VFalse -> false
  (* | VNot f -> constrained f target *)
  | VNot _ -> false (* e.g., view "!(msg.value = 0)" as not constrained *)
  ...
  | ForAll (_bvs,f) -> constrained f target
  | Label (_l,f) -> constrained f target
...
= fun state ve ->
  ...
  | VBinOp (vbop,e1,VInt n,typ) when (vbop = VMul || vbop = VDiv) && Z.equal n Z.zero ->
...
= fun vf ->
  ...
  let (state,_sc) = split_implication vf in
  trans_f state vf
