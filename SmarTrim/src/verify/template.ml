  ...
  List.exists (fun l ->
    ...
    | [NoOverflow (b_1,_);
      ...
      VBinRel (VGeq, VBinOp (VAdd, Read (VVar (b2,_), x4), Read (VVar (b3,_), y3), _),
                                   Read (VVar (b4,_), x5))
      when Z.lt (Z.add n n) max_256bit &&
           total1 = total2 &&
...
= fun vf ->
  ...
  List.exists (fun l ->
    ...
    | [NoOverflow (b1,_);
       ...
       VBinRel (VEq, VVar (total2,_), VInt n);
       VBinRel (VGeq, Read (VVar (b3,_), _), v1)
      ],
...
= fun vf ->
  ...
  List.exists (fun l ->
    ...
    | [NoOverflow (b1,_);
      ...
      VBinRel (VGeq, VBinOp (VAdd, Read (VVar (b3,_), x1), Read (VVar (b4,_), _), _),
                                   Read (VVar (b5,_), x2))
      when Z.lt (Z.add n n) max_256bit &&
           total1=total2 &&
 ...
 *  G * 10 is safe.
 * *)
(* XXX: e.g., cve-2018-14004, line 280 *)
...
= fun vf ->
  ...
  List.exists (fun l ->
    ...
    | [VBinRel (rel1, Read (VVar (b_1,_), f1), v1);
      ...
      -> true
    (* XXX: e.g., cve-2018-11239, line 85 *)
 ...
 *  m >= v is safe.
 * *)
(* XXX: e.g., cve-2018-14004, line 281 *)
  ...
  List.exists (fun f ->
    ...
    | VBinRel (VEq,e1,VInt n1),
      VOr (VBinRel (VEq,e2,VInt n2),
           VAnd (VNot (VBinRel (VEq,e3,VInt n3)), VBinRel (VEq, VBinOp (VDiv, (VBinOp (VMul,e4,_,_)),e6,_), _))) ->
      Z.equal n1 Z.one && Z.equal n2 Z.zero && Z.equal n3 Z.zero &&
...
= fun vf ->
  ...
  List.exists (fun f ->
    match f,con with
    | VBinRel (VEq,e1,VInt n0), VBinRel (VGeq,VBinOp (VAdd,e2,e3,_typ),e4)
      when equal_ve e2 e4 && Z.equal n0 Z.zero ->
...
= fun vf ->
  ...
  List.exists (fun l ->
    match l,con with
    | [VBinRel (VEq,e1,VBinOp(VDiv,_e2,e3,_));
      ...
      when Z.equal n4 Z.zero && Z.equal n5 Z.zero && Z.equal n6 Z.zero &&
...
let template12 : vformula -> bool
= fun vf ->
  let (pre,con) = split_implication vf in
  let lst_3 = pre |> list_of_conjuncts |> cartesian_3 in
  List.exists (fun l ->
    match l,con with
    | [VBinRel (VGeq, VBinOp (VAdd, Read (VVar (b_1,_), x1), Read (VVar (b_2,_), y1), _),
                                    Read (VVar (b_3,_), x2));
       VBinRel (VEq, VVar (b__1,_), Write (VVar (b_4,_), x3, VBinOp (VSub, Read (VVar (b_5,_), x4), v1, _)));
       VBinRel (VEq, VVar (b1,_), Write (VVar (b__2,_), y2, VBinOp (VAdd, Read (VVar (b__3,_), y3), v2, _)))
      ],
      VBinRel (VGeq, VBinOp (VAdd, Read (VVar (b2,_), x5), Read (VVar (b3,_), _y4), _),
                                   Read (VVar (b4,_), x6))
      when b_1 = b_2 && b_2 = b_3 && b_3 = b_4 && b_4 = b_5 &&
           b__1 = b__2 && b__2 = b__3 &&
           b1=b2 && b2=b3 && b3=b4 &&
           equal_ve x1 x2 && equal_ve x2 x3 && equal_ve x3 x4 && equal_ve x4 x5 && equal_ve x5 x6 &&
           equal_ve y1 y2 && equal_ve y2 y3 && equal_ve y2 y3 &&
           equal_ve v1 v2
      -> true
    | _ -> false
  ) lst_3
...
= fun vf ->
  ...
  | Imply (_,VBinRel (VGeq, (* e.g., v >= (v*99)/100 *)
    ...
    when BatString.equal v1 v2 && Z.lt n1 n2
    -> true
  | Imply (_,VBinRel (VGeq, (* e.g., (x%10) + 1 >= (x%10) *)
                      VBinOp (VAdd, VBinOp (VMod,_,VInt n1,_), VInt n2, t1),
                      VBinOp (VMod,_,VInt n3,t2)))
    when Z.geq (Z.add n1 n2) n3
         && Z.geq (Z.of_int 1000000000000000) (Z.add n1 n2)
         && Typ.is_uint256 t1 && Typ.is_uint256 t2
    -> true
  ...
  | Imply (_,VBinRel (VGeq, (* e.g., 140 + uint8(exp/42) *)
                      ...
                      VInt n3))
    when Z.geq (Z.add n1 n2) n3
         && Z.geq (Z.of_int 255) (Z.add n1 n2)
    -> true
  | Imply (_,VBinRel (VGeq, (* e.g., 50 + (exp/3) *)
    ...
    let added = Z.add n1 (Z.div max n2) in
    let test = Z.leq added max in
    test
  | Imply (_pre,_con) when template10 vf -> true
  | Imply (_pre,_con) when template11 vf -> true
  | Imply (_pre,_con) when template12 vf -> true
  | Imply (_pre,_con) ->
