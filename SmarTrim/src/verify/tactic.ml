...
= fun flst ->
  ...
  let flst2 = List.filter (fun vf ->
                ...
                | VBinRel (VEq, VVar _, Write (VVar _, x, VBinOp (VAdd, Read (VVar _, x1), _v, _))) -> equal_ve x x1
                | _ -> false
              ) flst in
  let flst3 = List.filter (fun vf ->
                ...
                | VBinRel (VEq, VVar _, Write (VVar _, x, VBinOp (VSub, Read (VVar _, x1), _v, _))) -> equal_ve x x1
                | VBinRel (VEq, VVar _, Write (VVar _, x, VBinOp (VAdd, Read (VVar _, x1), _v, _))) -> equal_ve x x1
                | _ -> false
              ) flst in
...
= fun flst ->
  ...
  let flst3 = List.filter (fun vf ->
                match vf with
                | VBinRel (VGeq,_,_) | VBinRel (VGt,_,_) -> true
                | VBinRel (VEq, VVar _, Write (VVar _, x, VBinOp (bop, Read (VVar _, x1), _v, _))) when bop=VAdd || bop=VSub -> equal_ve x x1
                | VBinRel (VEq, VVar _, Ite (VCond (VBinRel (VEq, Read (VVar _, _), VCond VFalse)), _, _)) -> true
                | _ -> false
              ) flst in
  let flst4 = List.filter (fun vf ->
...
= fun flst ->
  ...
  let flst4 = List.filter (fun vf ->
                match vf with
                | VBinRel (VEq, VVar _, Write (VVar _, x, VBinOp (bop, Read (VVar _, x1), _v, _))) when bop=VAdd || bop=VSub -> equal_ve x x1
                | VNot (VBinRel (VEq, _div1, VInt _zero)) -> true
                | VBinRel (VEq, VVar _, Ite (VCond (VBinRel (VEq, Read (VVar _, _), VCond VFalse)), _, _)) -> true
                | _ -> false
              ) flst in
  let flst5 = List.filter (fun vf ->
...
= fun vf ->
  ...
  List.fold_left (fun acc l ->
    match l with
    (* smartbugs: Collect @ 0x01f8c4e3fa3edeb29e514cba738d87ce8c091d3f *)
    | [UntrustSum (isum_, b);
       VBinRel (VGeq, VBinOp (VAdd, VVar isum_1, v, _), VVar isum_2);
       VBinRel (VEq, VVar isum, Ite (VCond (VBinRel (VEq, Read (VVar trust, _y), VCond VFalse)),
    ...
    (* smartbugs: reentrancy Collect function *)
  ...
  List.fold_left (fun acc l ->
    ...
    | [UntrustSum (isum, b_);
       ...
       when not (is_renamed (fst isum)) &&
            ...
            (brel = VGeq || brel = VGt)
       ->
       let true_x = List.exists (fun vf -> match vf with VBinRel (VEq, Read (VVar _trust_map, x'), VCond VTrue) when equal_ve x' x -> true | _ -> false) (list_of_conjuncts case1) in
       let true_y = List.exists (fun vf -> match vf with VBinRel (VEq, Read (VVar _trust_map, y'), VCond VTrue) when equal_ve y' y -> true | _ -> false) (list_of_conjuncts case1) in
       let false_x = List.exists (fun vf -> match vf with VBinRel (VEq, Read (VVar _trust_map, x'), VCond VFalse) when equal_ve x' x -> true | _ -> false) (list_of_conjuncts case2) in
       let false_y = List.exists (fun vf -> match vf with VBinRel (VEq, Read (VVar _trust_map, y'), VCond VFalse) when equal_ve y' y -> true | _ -> false) (list_of_conjuncts case2) in
...
= fun vf ->
  ...
  List.fold_left (fun acc l ->
    ...
    | [UntrustSum (isum_, b_);
       VBinRel (VEq, VVar isum, Ite (VCond (VBinRel (VEq, Read (VVar _trust, _y), VCond VFalse)),
                                     VBinOp (VAdd, VVar isum_1, msgvalue, _), VVar isum_2));
       VBinRel (VEq, v, VBinOp (VDiv, msgvalue1, div, _));
       VNot (VBinRel (VEq, div1, VInt zero));
       VBinRel (brel, Read (VVar b_1, x), v1);
       VBinRel (VEq, VVar b__, Write (VVar b_2, x1, VBinOp (VSub, Read (VVar b_3, x2), v2, _)));
       VBinRel (VEq, VVar b, Write (VVar b__1, _y1, VBinOp (VAdd, Read (VVar b__2, _y2), v3, _)))]
       when is_renamed (fst b_) && is_renamed (fst b__) && not (is_renamed (fst b)) &&
            b_ = b_1 && b_1 = b_2 && b_2 = b_3 &&
            b__ = b__1 && b__1 = b__2 &&
            equal_ve x x1 && equal_ve x1 x2 &&
            equal_ve msgvalue (VVar msg_value) && equal_ve msgvalue1 (VVar msg_value) &&
            equal_ve div div1 && Z.equal zero Z.zero &&
            equal_ve v v1 && equal_ve v1 v2 && equal_ve v2 v3 &&
            is_renamed (fst isum_) && not (is_renamed (fst isum)) &&
            is_renamed (fst isum_) && not (is_renamed (fst isum)) &&
            isum_ = isum_1 && isum_1 = isum_2 &&
    ...
    | [UntrustSum (isum_, b_);
       ...
       VNot (VBinRel (VEq, div1, VInt zero));
       VBinRel (brel, Read (VVar b_1, x), v1);
       VBinRel (VEq, VVar b__, Write (VVar b_2, _y1, VBinOp (VAdd, Read (VVar b_3, _y2), v2, _)));
    ...
    (* smartbugs reentrancy: simple dao *)
    | [UntrustSum (isum_, b_);
       VOr (case1, case2);
       VBinRel (brel1, VBinOp (VAdd, VVar isum_1, v, _), VVar isum_2);
       VBinRel (VEq, VVar isum, Ite (VCond (VBinRel (VEq, Read (VVar trust, x), VCond VFalse)),
