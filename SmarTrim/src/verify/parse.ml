...
(* owner__@2 -> owner *)
let peel_lab : Var.t -> Var.t
= fun (x,t) -> try (fst (BatString.split x ~by:"__@"), t) with Not_found -> (x,t)

let to_string_var (x,t) = x ^ ":" ^ Typ.to_string t
...
let rec to_string_vformula2 vf =
  ...
  | VOr (_f1,_f2) -> raise NotImplemented
  ...
  | Label (_,_f) -> raise NotImplemented
...
let parse_typ : string -> Typ.t
= fun str ->
  let trans_mapping2 : string -> Typ.t
  = fun str ->
    let _ = assert (BatString.starts_with str "mapping2") in
    let (left, right) = BatString.split str ~by:" => " in (* "mapping2(key => val)" -> ( "mapping2(key", "val)" )*)
...
let rec parse_vf : Var.t BatSet.t -> string -> vformula
= fun vars str ->
  match str with
  | "true" -> VTrue
  | "false" -> VFalse
...
and parse_ve : Var.t BatSet.t -> string -> vexp
= fun vars str ->
  match str with
...
let parse_spec : Var.t BatSet.t -> Yojson.Basic.t list -> SpecMap.t -> SpecMap.t
= fun vars lst spec0 ->
  let bindings = SpecMap.bindings spec0 in
  List.fold_left2 (fun acc elem (k,_) ->
    let pre_str = elem |> J.member "pre" |> J.to_string in
    let post_str = elem |> J.member "post" |> J.to_string in
    let pre = parse_vf vars pre_str in
    let post = parse_vf vars post_str in
    SpecMap.add k (pre,post) acc
  ) SpecMap.empty lst bindings

let parse_inv : Var.t BatSet.t -> Yojson.Basic.t list -> InvMap.t -> InvMap.t
= fun vars lst inv0 ->
  let bindings = InvMap.bindings inv0 in
  List.fold_left2 (fun acc elem (k,_) ->
    let f_str = elem |> J.member "formula" |> J.to_string in
    let f = parse_vf vars f_str in
    InvMap.add k f acc
  ) InvMap.empty lst bindings

let do_parse : Var.t BatSet.t -> string -> InvMap.t * SpecMap.t -> InvMap.t * SpecMap.t
= fun vars file (inv0,spec0) ->
  let json = Yojson.Basic.from_file file in
  let inv_json = json |> J.member "invmap" |> J.to_list in
  let spec_json = json |> J.member "specmap" |> J.to_list in
  let inv = parse_inv vars inv_json inv0 in
  let spec = parse_spec vars spec_json spec0 in
  (inv,spec)

let collect_comps (global : Global.t) paths =
  Paths.Set.fold (fun p acc ->
    let f = FuncMap.find p.fkey global.fmap in
    let cfg = Func.cfg f in
    let comps = Component.collect_bp global p.basic_path cfg in
    Component.union comps acc
  ) paths Component.empty_comps

let collect_vars : Global.t -> Paths.Set.t -> Var.t BatSet.t
= fun global paths ->
...
let run : Global.t -> Paths.Set.t -> string -> InvMap.t * SpecMap.t -> bool * InvMap.t * SpecMap.t
