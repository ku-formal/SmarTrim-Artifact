...
let rec contain_given_lock : Var.t -> vformula -> bool
= fun var vf ->
  match vf with
...
let rec contain_lock_s : vformula -> Stmt.t -> bool
= fun vf stmt ->
  match stmt with
  | Assign (Var (v,vinfo),_,_) when Typ.is_bool vinfo.vtyp -> contain_given_lock (v,vinfo.vtyp) vf
...
= fun vf ->
  ...
  | VOr (VBinRel (VEq,e1,VInt n1),
         VAnd (VNot (VBinRel (VEq,e2,VInt n2)), VBinRel (VEq, VBinOp (VDiv, (VBinOp (VMul,e3,e4,_t)),e5,_t'), e6))) ->
    Z.equal n1 Z.zero && Z.equal n2 Z.zero &&
...
(* if invariant candidate is unsat by itself, this may lose information *)
(* e.g., lock=true /\ lock=false -> x = 5 ~> true -> x=5 *)
let property_focused_simplification : vformula -> vformula
= fun vf ->
  let (pre,con) = split_implication vf in
  let dep_vars = Optimize.compute_dependant_vars pre con in 
  (* should consider path condtion as well, e.g., deadcode *)
  let pre' = Optimize.remove_unrelated_part dep_vars pre in
  Imply (pre',con)
...
= fun vf ->
  ...
  (* To ensure termination. Also, the last step is normalization (e.g., cannot be applied to udiv) *)
  let vf = vf |> normalize |> propagate_eq |> normalize |> propagate_eq in
  let vf = vf |> normalize |> propagate_eq |> normalize in

  let vf = Tactic.apply vf in
  (* TODO: handling stackoverflow cases *)
  ...
  if b then (true, None)
    ...
    if is_invalid_approx vf then (false, None)
      ...
      let stat, mop, _ = Z3i.Main.chk vf in
      stat = UNSATISFIABLE, mop
