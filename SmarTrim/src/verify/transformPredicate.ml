...
module PartitionSet = BatSet.Make (struct type t = ExpSet.t let compare = ExpSet.compare end) 
...
= fun s m ->
  let elemtyp = Typ.range (get_typ_vexp m) in
...
let rec get_addrs_f : Var.t -> vformula -> ExpSet.t
= fun x vf ->
  match vf with
...
and get_addrs_e : Var.t -> vexp -> ExpSet.t
= fun x ve ->
  match ve with
  | VInt _ -> ExpSet.empty
  | Str _ -> ExpSet.empty
...
= fun vf ->
  match vf with
  | ForAll (_, VBinRel (VEq, ve, VInt n))
    when Typ.is_uint256 (get_typ_vexp ve) && Z.equal n Z.zero ->
    let id = get_id ve "" in
    let elemtyp = get_typ_vexp ve in
    let rem_zero = VBinRel (VEq, VVar ("@R_" ^ id, elemtyp), VInt Z.zero) in
    let rem_no_flow = VBinRel (VEq, VVar ("@noflow_" ^ id, EType Bool), VCond VTrue) in
    let urem_zero = VBinRel (VEq, VVar ("@U_" ^ id, elemtyp), VInt Z.zero) in
