...
let str_transfer_sender_has_enough_money () =
  {%string|  function smartest_transfer(address to, uint256 value) external {
    require(msg.sender != to);
    uint pre_msg = %{!balance_name}[msg.sender];
    uint pre_to = %{!balance_name}[to];
    bool ret = transfer(to, value);
    require((ret && pre_msg >= value) || !ret, "valismartest_falsified");
  }|}
;;
...
let str_sender_transfer_sender_bal_dec () =
  ...
  \    require ((msg.sender != to));\n\
  \    uint pre_msg = " ^ !balance_name ^ "[msg.sender];\n    uint pre_to = " ^ !balance_name
  ^ "[to];\n    bool ret = transfer(to, value);\n    require ((((ret == true) && (" ^ !balance_name
  ^ "[msg.sender] == (pre_msg - value))) || ((ret == false) && (" ^ !balance_name
  ^ "[msg.sender] == pre_msg))), \"valismartest_falsified\");\n  }"
;;
...
let str_transfer_recipient_bal_inc () =
  ...
  \    uint pre_msg = " ^ !balance_name ^ "[msg.sender];\n    uint pre_to = " ^ !balance_name
  ^ "[to];\n    bool ret = transfer(to, value);\n    require ((((ret == true) && (" ^ !balance_name
  ^ "[to] == (pre_to + value))) || ((ret == false) && (" ^ !balance_name
  ^ "[to] == pre_to))),  \"valismartest_falsified\");\n  }"
;;
...
let str_transferFrom_bal_enough () =
  ...
  ^ "[to];\n    uint pre_msg = " ^ !allow_name
  ^ "[from][msg.sender];\n\
    \    bool ret = transferFrom (from, to, value);\n\
    \    require ((((ret == true) && (pre_from >= value)) || (ret == false)), \
     \"valismartest_falsified\");\n\
    \  }"
;;
...
let str_transferFrom_sender_allow_enough () =
  ...
  ^ "[from][msg.sender];\n\
    \    bool ret = transferFrom (from, to, value);\n\
    \    require ((((ret == true) && (pre_msg >= value)) || (ret == false)), \
     \"valismartest_falsified\");\n\
    \  }"
;;
...
let str_transferFrom_from_bal_dec () =
  ...
  ^ "[from][msg.sender];\n\
    \    bool ret = transferFrom (from, to, value);\n\
    \    require ((((ret == true) && (" ^ !balance_name
  ^ "[from] == (pre_from - value))) || ((ret == false) && (" ^ !balance_name
  ^ "[from] == pre_from))), \"valismartest_falsified\");\n  }"
;;
...
let str_transferFrom_sender_allow_dec () =
  ...
  ^ "[from][msg.sender];\n\
    \    bool ret = transferFrom (from, to, value);\n\
    \    require ((((ret == true) && (" ^ !allow_name
  ^ "[from][msg.sender] == (pre_msg - value))) || ((ret == false) && (" ^ !allow_name
  ^ "[from][msg.sender] == pre_msg))), \"valismartest_falsified\");\n  }"
;;
...
let str_balance_sum_no_overflow () =
  ...
  \    require ((addr1 != addr2));\n\
  \    require (((" ^ !balance_name ^ "[addr1] + " ^ !balance_name ^ "[addr2]) >= " ^ !balance_name
  ^ "[addr1]), \"valismartest_falsified\");\n  }"
;;

let str_total_ge_balance () =
  "  function smartest_balance_le_total (address addr) public {\n\
  \    require (addr != address (0));\n\
  \    require ((" ^ !total_name ^ " >= " ^ !balance_name
  ^ "[addr]), \"valismartest_falsified\");\n  }"
;;
...
let collect_queries : Global.t -> vformula -> Paths.t -> Stmt.t -> Query.t list =
 fun _global vf path stmt ->
  match stmt with
  | Assert (e, "default", loc) when !Chk.assert_ ->
  ...
  | Assert (e, "erc20", loc) when !Chk.erc20 ->
    let sc = Semantics.convert_bexp e in
    let vc = Imply (vf, sc) in
