...
contract AOIon_1Interface is TheAO {
	using SafeMath for uint256;
...
contract AOIon_1 is AOIon_1Interface {
	using SafeMath for uint256;
	...
	IAOIon_1Lot internal _aoIonLot;
	...
	constructor(string memory _name, string memory _symbol, address _settingTAOId, address _aoSettingAddress, address _nameTAOPositionAddress, address _namePublicKeyAddress, address _nameAccountRecoveryAddress)
		AOIon_1Interface(_name, _symbol, _nameTAOPositionAddress, _namePublicKeyAddress, _nameAccountRecoveryAddress) public {
	...
	function setAOIon_1LotAddress(address _aoIonLotAddress) public onlyTheAO {
		require (_aoIonLotAddress != address(0));
		aoIonLotAddress = _aoIonLotAddress;
		_aoIonLot = IAOIon_1Lot(_aoIonLotAddress);
	}
...
contract AOETH is TheAO, TokenERC20, tokenRecipient {
	...
	constructor(uint256 initialSupply, string memory tokenName, string memory tokenSymbol, address _aoIonAddress, address _nameTAOPositionAddress)
		...
		setAOIon_1Address(_aoIonAddress);
		setNameTAOPositionAddress(_nameTAOPositionAddress);
	}
	...
	function setAOIon_1Address(address _aoIonAddress) public onlyTheAO {
		require (_aoIonAddress != address(0));
		aoIonAddress = _aoIonAddress;
		_aoIon = AOIon_1(_aoIonAddress);
	}
...
contract AOIon is AOIon_1 {
    constructor() AOIon_1("rlf", "rlf", address(0x123), address(0x123), address(0x123), address(0x123), address(0x123)) public { }
}
