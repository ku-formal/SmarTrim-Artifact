...
contract Production_1Boiler is BasicAuth
{
    ...
    bool g_Synced = false;
    function SyncOldData(OldProduction_1Boiler oldBoiler, address[] accounts) external CreatorAble
    {
        require(!g_Synced);
        g_Synced = true;
        for (uint i=0; i<accounts.length; i++)
...
contract Production_1 is Child {
    ...
    Production_1Boiler g_Boilers;

    constructor(Main main, Production_1Boiler pb) public Child(main)
    {
        g_Boilers = pb;
...
contract Production is Production_1 {
    constructor(Main main, ProductionBoiler pb) Production_1(main, pb) public { }
}
