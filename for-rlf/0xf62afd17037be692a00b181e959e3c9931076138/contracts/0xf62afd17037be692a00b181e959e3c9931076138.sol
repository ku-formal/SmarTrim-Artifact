...
contract RC_1 {
    ...
    function RC_1(address _tokenSaleContract, uint256 _oneTokenInUsdWei, uint256 _remainingTokens,  uint256 _startTime , uint256 _endTime ) public {
        ...
        tokenSaleContract.addMeByRC_1();
        
        soldTokens = 0;
        remainingTokens = _remainingTokens;
        oneTokenInUsdWei = _oneTokenInUsdWei;
        
        setTimeRC_1( _startTime, _endTime );
    }
    
    function setTimeRC_1(uint256 _startTime, uint256 _endTime ) internal {
        if( _startTime == 0 ) {
    ...
    event BuyRC_1(address indexed buyer, bytes trackID, uint256 value, uint256 soldToken, uint256 valueTokenInUsdWei );
    ...
    function () public payable {
        ...
        uint256 tokenAmount = tokenSaleContract.buyFromRC_1.value(msg.value)(msg.sender, oneTokenInUsdWei, remainingTokens);
        
        remainingTokens = remainingTokens.sub(tokenAmount);
        soldTokens = soldTokens.add(tokenAmount);
        
        BuyRC_1( msg.sender, msg.data, msg.value, tokenAmount, oneTokenInUsdWei );
    }
}
...
contract CardSale {
    ...
    event NewRC_1(address contr);
    
    function addMeByRC_1() public {
    ...
    function newCard(uint256 _oneTokenInUsdWei) onlyTokenSaleOwner public {
        new RC_1(this, _oneTokenInUsdWei, remainingTokens, 0, 0 );
    }
    ...
    function buyFromRC_1(address _buyer, uint256 _rcTokenValue, uint256 ) onlyRC_1 public payable returns(uint256) {
        uint256 tokenAmount = tokenSaleContract.buyFromRC_1.value(msg.value)(_buyer, _rcTokenValue, remainingTokens);
        remainingTokens = remainingTokens.sub(tokenAmount);
        soldTokens = soldTokens.add(tokenAmount);
        return tokenAmount;
    }
...
contract TokenSale is Ownable {
    ...
    function buyFromRC_1(address _buyer, uint256 _rcTokenValue, uint256 _remainingTokens) onlyRC_1 isBuyable public payable returns(uint256) {
    ...
    function newRC_1(uint256 _oneTokenInUsdWei, uint256 _remainingTokens) onlyOwner public {
        new RC_1(this, _oneTokenInUsdWei, _remainingTokens, 0, 0 );
    }
}
    
contract RC is RC_1 {
    constructor() RC_1(address(0x123), 78, 78, 78, 78) public { }
}
