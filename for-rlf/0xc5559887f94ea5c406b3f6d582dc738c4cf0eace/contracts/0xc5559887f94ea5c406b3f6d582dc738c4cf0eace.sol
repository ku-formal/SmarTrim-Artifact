...
contract ETHLottery_1ManagerInterface {
    function register();
}

contract ETHLottery_1Interface {
    function accumulate();
}

contract ETHLottery_1 {
    bytes32 public name = 'ETHLottery_1 - Last 1 Byte Lottery';
    ...
    function ETHLottery_1(address _manager, uint256 _fee, uint256 _jackpot, uint256 _owner_fee, address _accumulated_from) {
        owner = msg.sender;
        open = true;
        ...
        if (_accumulated_from != owner) {
            ...
            ETHLottery_1Interface lottery = ETHLottery_1Interface(accumulated_from);
            lottery.accumulate();
        }
        
        ETHLottery_1ManagerInterface manager = ETHLottery_1ManagerInterface(manager_address);
        manager.register();
        Open(open);
    }
...
contract ETHLottery is ETHLottery_1 {
    constructor() ETHLottery_1(address(0x123), 78, 78, 78, address(0x123)) public { }
}
