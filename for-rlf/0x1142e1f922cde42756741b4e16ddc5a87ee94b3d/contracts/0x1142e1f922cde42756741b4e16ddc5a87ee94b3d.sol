...
contract Hodl_1 is CATContract {
    uint public instanceId = 1;
    mapping(uint => Hodl_1Instance) public instances;

    uint public maximumHodl_1Duration = 4 weeks;

    event Hodl_1Created(uint indexed id, address indexed instOwner, uint hodlAmount, uint endTime);
    event Hodl_1Withdrawn(uint indexed id, address indexed byWho, uint hodlAmount);

    event MaximumHodl_1DurationChanged(uint oldLimit, uint newLimit);

    struct Hodl_1Instance {
        uint instId;
        address instOwner;
    ...
    function Hodl_1(address _catPaymentCollector) CATContract(_catPaymentCollector, "Hodl_1") {}

    function createNewHodl_1(uint _endTime) external payable blockCatEntryPoint limitTransactionValue whenNotPaused returns (uint currentId) {
        
        require(_endTime >= now);
        
        require((_endTime - now) <= maximumHodl_1Duration);
        ...
        Hodl_1Instance storage curInst = instances[currentId];
        ...
        Hodl_1Created(currentId, instanceOwner, hodlAmount, endTime);
        instanceId++;
    }

    function withdraw(uint _instId) external onlyInstanceOwner(_instId) instanceExists(_instId) whenNotPaused {
        Hodl_1Instance storage curInst = instances[_instId];
        ...
        Hodl_1Withdrawn(_instId, msg.sender, curInst.hodlAmount);
    }

    function changeMaximumHodl_1Duration(uint _newLimit) external onlyOwner {
        MaximumHodl_1DurationChanged(maximumHodl_1Duration, _newLimit);
        maximumHodl_1Duration = _newLimit;
    }

    
    function getHodl_1Owner(uint _instId) constant external returns (address) {
        return instances[_instId].instOwner;
    }

    function getHodl_1HasBeenWithdrawn(uint _instId) constant external returns (bool) {
        return instances[_instId].hasBeenWithdrawn;
    }

    function getHodl_1Amount(uint _instId) constant external returns (uint) {
...
contract Hodl is Hodl_1 {
    constructor() Hodl_1(address(0x123)) public { }
}
