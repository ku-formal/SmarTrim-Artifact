...
contract GLDS_1 is IERC20, Ownable, ReentrancyGuard, Pausable  {
   using SafeMath for uint256;
    ...
    uint256 private _initSupply;
     ...
     mapping(address => Payment) internal awaitGLDS_1;
    ...
    function transferTokens(uint256 rateX, uint256 rateY) public onlyAdmin returns (bool) {
      ...
      uint256 qtyWei = awaitGLDS_1[keyList[i]].valueETH;
      uint256 qtyGLDS_1 = qtyWei.div(rateY).mul(rateX);
      uint256 FromWalletB25 = (balanceOf(_walletB25) <= qtyGLDS_1)? balanceOf(_walletB25) : qtyGLDS_1;
      qtyGLDS_1 -= FromWalletB25;
      uint256 FromWalletB20 = (balanceOf(_walletB20) <= qtyGLDS_1)? balanceOf(_walletB20) : qtyGLDS_1;
      qtyGLDS_1 -= FromWalletB20;
      uint256 FromWalletB10 = (balanceOf(_walletB10) <= qtyGLDS_1)? balanceOf(_walletB10) : qtyGLDS_1;
      qtyGLDS_1 -= FromWalletB10;
      ...
      qtyWei = qtyGLDS_1.div(rateX).mul(rateY);
      awaitGLDS_1[keyList[i]].valueETH = qtyWei;
      }
      return true;
    }

    
    function setWalletFund(address WalletFund) public onlyOwner returns (bool){
    ...
    function addPay(address _key, uint256 _valueETH) internal {
        Payment storage pay = awaitGLDS_1[_key];
    ...
    function removePay(address _key) public onlyOwner returns (bool){
        ...
        awaitGLDS_1[keyList[keyListLastIndex]].index = keyListIndex + 1;
        keyList[keyListIndex] = keyList[keyListLastIndex];
        keyList.length--;
        delete awaitGLDS_1[_key];
        return true;
    }
    ...
    function sumAwaitingList() public view returns (uint256) {
        ...
        uint256 sum = 0;
        for (uint i = 0; i < len; i++) {
        sum += awaitGLDS_1[keyList[i]].valueETH;
        }
        return uint256(sum);
    }
    ...
    function containsAwaitingList(address _key) public view returns (bool) {
        return awaitGLDS_1[_key].index > 0;
    }

    function getIndex(address _key) public view returns (uint256) {
        return awaitGLDS_1[_key].index;
    }

   function getValueETH(address _key) public view returns (uint256) {
        return awaitGLDS_1[_key].valueETH;
    }

    function getByIndex(uint256 _index) public view returns (uint256, address, uint256) {
        require(_index >= 0);
        require(_index < keyList.length);
        return (awaitGLDS_1[keyList[_index]].index, keyList[_index], awaitGLDS_1[keyList[_index]].valueETH);
    }

    function addB25(address _key, uint256 _valueGLDS_1) internal {
        B25 storage bonus25 = awaitB25[_key];
        bonus25.valueGLDS_1 += _valueGLDS_1;
        if(bonus25.index > 0){ 
    ...
    function sumB25List() public view returns (uint256) {
        ...
        uint256 sum = 0;
        for (uint i = 0; i < len; i++) {
        sum += awaitB25[keyListB25[i]].valueGLDS_1;
        }
        return uint256(sum);
    }
   ...
   function getB25ValueGLDS_1(address _key) public view returns (uint256) {
        return awaitB25[_key].valueGLDS_1;
    }

    function getB25ByIndex(uint256 _index) public view returns (uint256, address, uint256) {
        require(_index >= 0);
        require(_index < keyListB25.length);
        return (awaitB25[keyListB25[_index]].index, keyListB25[_index], awaitB25[keyListB25[_index]].valueGLDS_1);
    }

    function addB20(address _key, uint256 _valueGLDS_1) internal {
        B20 storage bonus20 = awaitB20[_key];
        bonus20.valueGLDS_1 += _valueGLDS_1;
        if(bonus20.index > 0){ 
    ...
    function sumB20List() public view returns (uint256) {
        ...
        uint256 sum = 0;
        for (uint i = 0; i < len; i++) {
        sum += awaitB20[keyListB20[i]].valueGLDS_1;
        }
        return uint256(sum);
    }
    ...
    function getB20ValueGLDS_1(address _key) public view returns (uint256) {
        return awaitB20[_key].valueGLDS_1;
    }

    function getB20ByIndex(uint256 _index) public view returns (uint256, address, uint256) {
        require(_index >= 0);
        require(_index < keyListB20.length);
        return (awaitB20[keyListB20[_index]].index, keyListB20[_index], awaitB20[keyListB20[_index]].valueGLDS_1);
    }

    function addB10(address _key, uint256 _valueGLDS_1) internal {
        B10 storage bonus10 = awaitB10[_key];
        bonus10.valueGLDS_1 += _valueGLDS_1;
        if(bonus10.index > 0){ 
    ...
    function sumB10List() public view returns (uint256) {
        ...
        uint256 sum = 0;
        for (uint i = 0; i < len; i++) {
        sum += awaitB10[keyListB10[i]].valueGLDS_1;
        }
        return uint256(sum);
    }
    ...
    function getB10ValueGLDS_1(address _key) public view returns (uint256) {
        return awaitB10[_key].valueGLDS_1;
    }

    function getB10ByIndex(uint256 _index) public view returns (uint256, address, uint256) {
        require(_index >= 0);
        require(_index < keyListB10.length);
        return (awaitB10[keyListB10[_index]].index, keyListB10[_index], awaitB10[keyListB10[_index]].valueGLDS_1);
    }
    
}

    
contract GLDS is GLDS_1 {
    constructor() GLDS_1("rlf", "rlf", 78, 78) public { }
}
