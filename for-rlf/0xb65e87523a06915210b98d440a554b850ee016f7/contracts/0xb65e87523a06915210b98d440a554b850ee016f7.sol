...
contract Bulls_1 is CryptoTeam {
    constructor(address payable _bankAddress, address payable _CBCTokenAddress) public {
        BankContract = Bank(_bankAddress);
        BankContract.setBulls_1Address(address(this));
        CBCTokenContract = CBCToken(_CBCTokenAddress);
        CBCTokenContract.approve(_bankAddress, 9999999999999999999000000000000000000);
    }
}
...
contract Bank {
    ...
    mapping (address => uint256) public depositBulls_1;
    uint256 public currentDeadline;
    uint256 public lastDeadline = 1546257600;
    uint256 public countOfBears;
    uint256 public countOfBulls_1;
    uint256 public totalSupplyOfBulls_1;
    uint256 public totalSupplyOfBears;
    uint256 public totalCBCSupplyOfBulls_1;
    uint256 public totalCBCSupplyOfBears;
    uint256 public probabilityOfBulls_1;
    ...
    Bulls_1 public Bulls_1Contract;
    CBCToken public CBCTokenContract;
    ...
    function setBulls_1Address(address payable _bullsAddress) external {
        require(address(Bulls_1Contract) == address(0x0));
        Bulls_1Contract = Bulls_1(_bullsAddress);
    }
    ...
    function setInfo(address _lastHero, uint256 _deposit) public {
        require(address(BearsContract) == msg.sender || address(Bulls_1Contract) == msg.sender);

        if (address(BearsContract) == msg.sender) {
            require(depositBulls_1[_lastHero] == 0, "You are already in bulls team");
        ...
        if (address(Bulls_1Contract) == msg.sender) {
            require(depositBears[_lastHero] == 0, "You are already in bears team");
            if (depositBulls_1[_lastHero] == 0)
                countOfBulls_1++;
            totalSupplyOfBulls_1 = totalSupplyOfBulls_1.add(_deposit.mul(90).div(100));
            depositBulls_1[_lastHero] = depositBulls_1[_lastHero].add(_deposit.mul(90).div(100));
        }
    ...
    function calculateProbability() public {
        ...
        totalCBCSupplyOfBulls_1 = CBCTokenContract.balanceOf(address(Bulls_1Contract));
        totalCBCSupplyOfBears = CBCTokenContract.balanceOf(address(BearsContract));
        uint256 percent = (totalSupplyOfBulls_1.add(totalSupplyOfBears)).div(100);

        if (totalCBCSupplyOfBulls_1 < 1 ether) {
            totalCBCSupplyOfBulls_1 = 0;
        }
        ...
        if (totalCBCSupplyOfBulls_1 <= totalCBCSupplyOfBears) {
            uint256 difference = totalCBCSupplyOfBears.sub(totalCBCSupplyOfBulls_1).div(0.01 ether);
            ...
            probabilityOfBulls_1 = 10000 - probabilityOfBears;
        } else {
            uint256 difference = totalCBCSupplyOfBulls_1.sub(totalCBCSupplyOfBears).div(0.01 ether);
            probabilityOfBulls_1 = totalSupplyOfBulls_1.mul(100).div(percent).add(difference);

            if (probabilityOfBulls_1 > 8000) {
                probabilityOfBulls_1 = 8000;
            }
            if (probabilityOfBulls_1 < 2000) {
                probabilityOfBulls_1 = 2000;
            }
            probabilityOfBears = 10000 - probabilityOfBulls_1;
        }
    ...
    function getWinners() public {
        ...
        uint256 seed2 = totalSupplyOfBulls_1;
        uint256 seed3 = totalSupplyOfBears;
        uint256 seed4 = totalCBCSupplyOfBulls_1;
        uint256 seed5 = totalCBCSupplyOfBulls_1;
    ...
    function () external payable {
        if (msg.value == 0 &&  !getState() && winner > 0){
            require(depositBears[msg.sender] > 0 || depositBulls_1[msg.sender] > 0);
            ...
            if (winner == 2 && depositBulls_1[msg.sender] > 0) {
                payout = calculateETHPrize(msg.sender);
            }

            if (payout > 0) {
                depositBears[msg.sender] = 0;
                depositBulls_1[msg.sender] = 0;
                msg.sender.transfer(payout);
            }

            if ((winner == 1 && depositBears[msg.sender] == 0) || (winner == 2 && depositBulls_1[msg.sender] == 0)) {
                payoutCBC = calculateCBCPrize(msg.sender);
                if (CBCTokenContract.balanceOf(address(Bulls_1Contract)) > 0)
                    CBCTokenContract.transferFrom(
                        address(Bulls_1Contract),
                        address(this),
                        CBCTokenContract.balanceOf(address(Bulls_1Contract))
                    );
    ...
    function calculateETHPrize(address participant) public view returns(uint) {
        ...
        uint payout = 0;
        uint256 totalSupply = (totalSupplyOfBears.add(totalSupplyOfBulls_1));

        if (depositBears[participant] > 0) {
            payout = totalSupply.mul(depositBears[participant]).div(totalSupplyOfBears);
        }

        if (depositBulls_1[participant] > 0) {
            payout = totalSupply.mul(depositBulls_1[participant]).div(totalSupplyOfBulls_1);
        }
    ...
    function calculateCBCPrize(address participant) public view returns(uint) {
        ...
        uint payout = 0;
        uint totalSupply = (totalCBCSupplyOfBears.add(totalCBCSupplyOfBulls_1)).mul(80).div(100);
...
contract Bulls is Bulls_1 {
    constructor() Bulls_1(address(0x123), address(0x123)) public { }
}
